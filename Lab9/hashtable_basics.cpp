...
struct HashTable{
	...
	//add your code below
        for(int i = 0; i < user_name.length(); i++){
            sum += user_name[i];  //Getting the sum of ASCII codes of each character
        }	
	...
	//add an if condition to complete the code here
        if(empty){
            //If slot is empty insert the password to the hash table
            password[hash] = user_password;
            cout<<"Password added successfully\n";
        }
        else
            //If slot is full
            cout<<"Can't insert the password,Slot is full\n";       
	...
	//add an if condition to complete the code here
        if(!empty){
            ...
            cout<<"There is no password for "<<user_name<<endl;
        }		
